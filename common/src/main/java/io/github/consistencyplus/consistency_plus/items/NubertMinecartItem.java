	...
	public ActionResult useOnBlock(ItemUsageContext context) {
		...
		if (!world.isClient) {
			...
			if (blockState.getBlock() instanceof AbstractRailBlock) {
				AbstractRailBlock rail = (AbstractRailBlock) blockState.getBlock();
				railShape = blockState.get(rail.getShapeProperty());
			} else {
			...
			cart.setCustomBlock((wigged ? CPlusBlocks.WIGGED_NUBERT : CPlusBlocks.NUBERT).getStateManager().getDefaultState());
